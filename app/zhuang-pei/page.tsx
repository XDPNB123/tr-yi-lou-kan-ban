  ...
  return (
    <>
      <div className="w-full h-full">
        ...
        >
          <CarouselContent>
            {cardData?.["装配"]?.map((item: any, index: number) => (
              <CarouselItem
                ...
                <MagicCard className="w-[24vw] h-[33vh]">
                  <div className="w-full h-full text-center">
                    <div className="text-4xl mt-4 text-slate-300">
                      {cardData?.["装配"]?.[index]?.work_center_name}
                    </div>
                    <div className="relative flex justify-center mt-8">
                      <WavePercent
                        percent={cardData?.["装配"]?.[index]?.percent}
                      ></WavePercent>
                    </div>
                    <div className="text-2xl flex w-full px-6 justify-between absolute bottom-4">
                      <span className=" text-slate-300">
                        ...
                        <span className="text-green-300">
                          {cardData?.["装配"]?.[index]?.completed}
                        </span>
                      </span>
                      <span className="text-slate-300">
                        总排产数量：
                        <span className="text-red-300">
                          {cardData?.["装配"]?.[index]?.qutstanding}
                        </span>
                      </span>
                    </div>
                  </div>
